...
namespace myplug
{
	namespace envelope
	{
		...
		class PointEditListener
		{
		public:
			virtual ~PointEditListener() = default;
			virtual void handleMoved(PointEdit* p) = 0;
		};

		class PointEdit : public juce::AnimatedAppComponent
		{
			EnvelopeManager* envManager_;
			std::function<void()> callbackWhenShownRangeChanged_;
			std::vector<PointEditListener*> listener_;
		...
		public:
			PointEdit(EnvelopeManager* envManager);
			...
			void addListener(PointEditListener* listener) { listener_.push_back(listener); }
			void removeListener(PointEditListener* listener) { listener_.erase(std::remove(listener_.begin(), listener_.end(), listener), listener_.end()); }
		};
	}
}