		...
		for (int k = 0; k < pointsPtr->getNumPoint(); ++k)
		{
			...
			case InterpolationType::Sigmoid:
			{
				...
				else
				{
					juce::Path stroke;
					juce::PathStrokeType strokeType(linethickness, juce::PathStrokeType::JointStyle::curved);
					stroke.startNewSubPath(calcScreenXPos(prevX), calcScreenYPos(envgen_->getInterpolatedValue(prevX)));

					while (length >= 0)
					{
						stroke.lineTo(calcScreenXPos(nextX), calcScreenYPos(envgen_->getInterpolatedValue(nextX)));
